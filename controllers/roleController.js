...
const roleRegister = async (req, res) => {
  try {
    ...
    //verify idCard
    const verifyRole = await Role.findOne({ name: name })
    if (verifyRole) {
      return res.status(400).json({ message: `El role ${verifyRole.name} ya existe` })
    }
    const newRole = await Role.create({ name, options })
    console.log('User created successfully', newRole)
    return res.status(200).json({ content: newRole })
  } catch (error) {
    if (error) {
...
module.exports = { getDataUser, updateUser, roleRegister, getUserPackage }
