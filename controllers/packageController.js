...
try {
  ...
  return res
    .status(200)
    .json({ package: await Package.find({ _id: response._id }).populate("services.service") });
  ...
  if (error) {
    return res
      .status(400)
      .send({ success: false, error: "Ocurrio un problema con el servidor" });
  }
  throw error;
}
}
 ...
 const updateUserPackage = async (req, res) => {
  ...
  try {
    ...
    await Package.updateMany(
      {},
      { $pull: { users: {$in: [mongoose.Types.ObjectId(userId)] }} }
    ) 
